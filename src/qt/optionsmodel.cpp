...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BON);
    ...
    if (!settings.contains("nAnonymizeBonyAmount"))
        settings.setValue("nAnonymizeBonyAmount", 1000);

    nAnonymizeBonyAmount = settings.value("nAnonymizeBonyAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBonyAmount"))
        SoftSetArg("-anonymizebonyamount", settings.value("nAnonymizeBonyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bony.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBonyAmount:
            return QVariant(nAnonymizeBonyAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBonyAmount:
            nAnonymizeBonyAmount = value.toInt();
            settings.setValue("nAnonymizeBonyAmount", nAnonymizeBonyAmount);
            emit anonymizeBonyAmountChanged(nAnonymizeBonyAmount);
            break;
        case CoinControlFeatures:
