...
#include "ui_zboncontroldialog.h"
...
std::set<std::string> ZBonControlDialog::setSelectedMints;
std::set<CMintMeta> ZBonControlDialog::setMints;

bool CZBonControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBonControlDialog::COLUMN_DENOMINATION || column == ZBonControlDialog::COLUMN_VERSION || column == ZBonControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBonControlDialog::ZBonControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBonControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBonControlDialog::~ZBonControlDialog()
{
    delete ui;
}

void ZBonControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBonControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBonControlWidgetItem* itemDenom(new CZBonControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBonControlWidgetItem *itemMint = new CZBonControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZBonControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBonControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBon_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBonControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBonControlDialog::GetSelectedMints()
{
...
void ZBonControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
