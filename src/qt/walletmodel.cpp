...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bony address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BON.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bony:URI (bony:XyZ...?message=example)
            {
...
static void NotifyzBONReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBONReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBONReset.connect(boost::bind(NotifyzBONReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBONReset.disconnect(boost::bind(NotifyzBONReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbonTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
