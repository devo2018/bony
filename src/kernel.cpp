...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZBonStake(spend));
    } else {
        ...
        CBonStake* bonInput = new CBonStake();
        bonInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(bonInput);
    }
