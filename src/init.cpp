...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bony.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bonyd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BON and zBON money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BON/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BON/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, bony, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BON/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BONY specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-bonstake=<n>", strprintf(_("Enable or disable staking functionality for BON inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbonstake=<n>", strprintf(_("Enable or disable staking functionality for zBON inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbon=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBON minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbonbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBON backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebonyamount=<n>     " + strprintf(_("Keep N BON anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The BONY Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that BONY is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BONY Core is shutting down."));
...
#endif
    // Make sure only a single BONY process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BONY Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BONY: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BONY specific: zerocoin and spork DB's
                ...
                // BONY: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBONMinted();
                        RecalculateZBONSpent();
                    }
                    RecalculateBONSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // BONY: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BONY Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BONY Core to complete") << "\n";
        ...
        zwalletMain = new CzBONWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBONWallet
        uiInterface.InitMessage(_("Syncing zBON wallet..."));

        bool fEnableZBonBackups = GetBoolArg("-backupzbon", true);
        pwalletMain->setZBonAutoBackups(fEnableZBonBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbonTracker->Init();
...
//    }
//
//    nAnonymizeBonyAmount = GetArg("-anonymizebonyamount", 0);
//    if (nAnonymizeBonyAmount > 999999) nAnonymizeBonyAmount = 999999;
//    if (nAnonymizeBonyAmount < 2) nAnonymizeBonyAmount = 2;
    ...
    LogPrintf("Anonymize BONY Amount %d\n", nAnonymizeBonyAmount);
    ...
    /* Denominations
       ...
       For example:
       1BON+1000 == (.1BON+100)*10
       10BON+10000 == (1BON+1000)*10
    */
