...
CzBONWallet::CzBONWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbon seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBONSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBONSeed_deprecated()) {
                    LogPrintf("%s: Updated zBON seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbon seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbon wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBONWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBONCount(nCountLastUsed);
    else if (!walletdb.ReadZBONCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBONWallet::Lock()
{
    seedMaster = 0;
}

void CzBONWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBONWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBON(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBONWallet::LoadMintPoolFromDB()
{
...
void CzBONWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBONWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBONWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbonTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBONWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBON(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbonTracker which also adds to database
    pwalletMain->zbonTracker->Add(dMint, true);
...
void CzBONWallet::SeedToZBON(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBONWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBONWallet::GenerateDeterministicZBON(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBONWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBONWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
