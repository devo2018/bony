...
#ifndef BONY_ZBONWALLET_H
#define BONY_ZBONWALLET_H
...
public:
    CzBONWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBON(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBON(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BONY_ZBONWALLET_H
