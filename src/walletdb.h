...
public:
    ...
    bool WriteZBONSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBONSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBONSeed_deprecated(uint256& seed);
    bool EraseZBONSeed();
    bool EraseZBONSeed_deprecated();

    bool WriteZBONCount(const uint32_t& nCount);
    bool ReadZBONCount(uint32_t& nCount);
